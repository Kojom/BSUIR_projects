...
namespace Assets.Scripts
{
    public class PlayerControl : MonoBehaviour
    {
        ...
        private DoorInteraction m_door;
        private bool m_nearDoor = false;
        ...
        private void Start()
        {
            ...
            Debug.Log("Last checkpoint: " + gm.m_lastCheckPoint);
        }
        ...
        private void Move()
        {
            ...
            if (m_isOnGround && !m_isOnSlope && !m_isJumping)
            { // On Ground
            ...
            else if (m_isOnGround && m_isOnSlope && !m_isJumping)
            { // On slope
        ...
        private void Interactions()
        {
            if (Input.GetKeyDown(KeyCode.E))
            {
                ...
                if (m_nearDoor && m_door.m_state)
                {
                    gm.m_lastCheckPoint = new Vector2(-32, -29);
                    m_door.NextLevel();
                }
            }
        }
        ...
        private void OnTriggerEnter2D(Collider2D other)
        {
            ...
            if (other.tag == "Door" && !m_door)
            {
                m_nearDoor = true;
                m_door = other.GetComponent<DoorInteraction>();
            }
        }
        ...
        private IEnumerator UpdateState()
        {
            while (true)
            {
                ...
                yield return new WaitUntil(() => (currentState = m_animator.GetCurrentAnimatorClipInfo(0)).Length > 0);
